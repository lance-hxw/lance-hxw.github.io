...
线程本地分配缓冲区
在eden中， 给每个java 预先分配一块私有空间， 加速对象分配， 减少多线程锁的开销

任何线程都优先在自己的TLAB分配， 如果不足， 就触发refill， 申请新的TLAB
- 如果分配失败
	- 在eden公共区争抢锁进行分配
	- 或者minorGC

注意， 在minorgc后， eden会被清理， 所以相关TLAB也会被重新分配（里面的对象去survior了）

其他部分由于不涉及对象分配， 所以不需要TLAB
